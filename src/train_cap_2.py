...
from ai.utils import Mushrooms, get_dataset_from_path
...
def train_model(path):
    train, test, num_classes = get_dataset_from_path(path)
    test_ds = Mushrooms(test)

    m = ResNet101Model(num_classes=num_classes)
    m.model
    ...
    for fold, (train_fold, val_fold) in enumerate(kf.split(X=train, y=train['label']), start=1):
        ...
        train_ds = Mushrooms(train_df, augmentations=train_augmentations)
        val_ds = Mushrooms(val_df, augmentations=val_augmentations)
        test_ds = Mushrooms(test, augmentations=val_augmentations)
